class TextBox
  ...
  def initialize(
    ...
    @text_sprite = Sprite.new(
      ...
      Image.new(@width * 0.94, @height * 0.8).draw_font_ex(0, 0, @text, @font, { color: @font_color })
    )
    @cursor = Sprite.new(
      @x + @width * 0.03,
      @y + @height * 0.1,
  ...
  def update
    ...
    if w > @text_sprite.image.width
      ...
      @text_sprite.image = Image.new(@width * 0.94, @height * 0.8).draw_font_ex(-diff, 0, @text, @font, { color: @font_color })
    else
      @cursor.x = @frame.x + @width * 0.03 + w
      @text_sprite.image = Image.new(@width * 0.94, @height * 0.8).draw_font_ex(0, 0, @text, @font, { color: @font_color })
    end
  end
